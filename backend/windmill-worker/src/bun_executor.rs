...
/// Dependency version overrides for security patches.
/// These inject npm/bun "overrides" into package.json to force patched versions
/// of transitive dependencies with known vulnerabilities.
const DEPENDENCY_OVERRIDES: &[(&str, &str)] = &[
    ("form-data", "4.0.4"), // CVE-2025-7783
];

fn inject_dependency_overrides(pkg_json: &str) -> String {
    if DEPENDENCY_OVERRIDES.is_empty() {
        return pkg_json.to_string();
    }

    let Ok(mut pkg) = serde_json::from_str::<serde_json::Value>(pkg_json) else {
        return pkg_json.to_string();
    };
    ...
    if let Some(overrides_obj) = overrides.as_object_mut() {
        for (name, version) in DEPENDENCY_OVERRIDES {
            overrides_obj.insert(name.to_string(), serde_json::json!(version));
        }
    }

    serde_json::to_string(&pkg).unwrap_or_else(|_| pkg_json.to_string())
}

pub async fn prepare_job_dir(reqs: &str, job_dir: &str) -> Result<()> {
    let (pkg, lock, empty, is_binary) = split_lockfile(reqs);
    let pkg = inject_dependency_overrides(pkg);
